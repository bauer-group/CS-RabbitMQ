...
SECRETS: dict[str, tuple[int, str]] = {
    # 32 bytes (64 hex chars) — Erlang distribution cookie (cluster auth secret)
    "RABBITMQ_ERLANG_COOKIE":  (32, "Erlang distribution cookie (cluster auth)"),

    # 16 bytes (32 hex chars) — broker account passwords
    "RABBITMQ_ADMIN_PASSWORD": (16, "RabbitMQ administrator password"),
    "APP_PASSWORD":            (16, "Application user password (init topology)"),
    "MONITORING_PASSWORD":     (16, "Monitoring user password (init topology)"),
}
...
def main(argv: list[str] | None = None) -> int:
    ...
    print()
    print("next: review remaining settings (hostnames, sizing preset, plugin toggles, ...)")
    print("      then start the stack with `docker compose -f docker-compose.development.yml up -d --build`")
    return 0
